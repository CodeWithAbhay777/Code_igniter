...
const Chatbox = ({ chatBoxVisibility, socket, username }) => {
  ...
  return (
    <div className={`fixed ${chatBoxVisibility ? `right-0 top-0` : `right-[-100rem] top-0 `}  h-full w-[25rem] lg:w-[25rem] md:w-[20rem] sm:w-[18rem] bg-gray-950 rounded shadow-[0px_0px_20px_rgba(0,0,0,1)] transition-all ease-in-out delay-3050`}>
      <div id='upperChatDiv' className='w-full h-[calc(100%-10%)] bg-gray-700 p-4 overflow-y-auto overflow-x-hidden scrollbar-thin scrollbar-webkit text-white flex-column'>

        {newMessage.map((e) => {

          return e.isAuthor ? <div className='h-[5rem] w-full bg-gray-300 my-2'>{e.displayMessage}</div> :
