...
const EditWindow = ({ setEditWindowState, savedData , setSavedRefresh }) => {
    ...
    return (
        <div className='fixed inset-0 bg-black bg-opacity-30 backdrop-blur-sm flex justify-center items-center z-[100] px-16 py-8'>
            <div className='h-full w-full bg-black rounded-lg p-1  shadow-[0px_0px_50px_rgba(0,0,0,1)] flex items-center justify-between flex-wrap'>
                <div className='h-full w-[75%] p-2 flex flex-col'>
                    <div className='bg-gray-900 w-full h-[4rem] rounded-lg flex items-center justify-between p-2'>
                        <div className=' h-full flex items-center gap-3 ' >
                            ...
                            <button
                                ...
                                className='h-[2.5rem] w-[10rem] bg-gray-800 rounded-md text-white hover:bg-gray-700 cursor-pointer' >Clear
                            </button>
                        </div>

                        <RxCross1 className='text-white text-lg mr-2 cursor-pointer hover:text-red-600' onClick={() => setEditWindowState(false)} />
                    </div>

                    <div className=' max-h-[92%] w-full flex-grow flex items-center justify-between p-2'>
                        <div className='h-full w-1/2'>
                            <CodeEditor language={savedData.newLangValue} input={savedData.newInputValue} onChange={(e) => setEditInput(e)} />
                        </div>

                        <div className={`h-full w-[49%] bg-gray-900 rounded-lg p-4 overflow-auto break-words text-gray-300 ${editIsError.current ? `text-red-500 border border-red-500` : `text-gray-300 border border-green-300`} ${!editResetBtrClr ? `border-0 text-gray-300` : null}`}>
